...
import { main as mainLlamaindexRag } from './index_llamaindex_rag.js';
...
rl.question(promptQuestion, (answer) => {
    switch (answer) {
        ...
        case '4':
            ...
            break;
        case '5':
            console.log('Running Llamaindex (Llama RAG)...');
            mainLlamaindexRag().then(() => {
                console.log('Llamaindex (Llama RAG) executed successfully.');
                rl.close();
            }).catch(err => {
                console.error('Error executing Llamaindex (Llama RAG):', err);
                rl.close();
            });
            break;
        default:
